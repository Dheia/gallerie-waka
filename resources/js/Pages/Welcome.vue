<template>
    ...
    <div class="dark:text-gray-300 text-gray-600 bg-white dark:bg-mydark">
        <div ref="top" class="relative flex items-top justify-center min-h-screen w-full sm:items-center sm:pt-0">
            <div class="w-full">
                <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div class="flex flex-wrap pt-2 lg:pt-16 lg:mb-8">
                        <div class="flex justify-center flex-wrap items-center sm:justify-start sm:pt-0">
                    ...
                    <NavigationBar id="myanchor" :searchDatas="queriesToSend" @toogleBio="toogleBio()" :showBio="showBio" @openOver="openOrCloseOver(true)" class="w-full my-8" />
                            ...
                            <div class="flex flex-wrap items-center justify-center space-x-4 font-heading">
                                <div class="">
                                    <div class="flex items-center relative w-max cursor-pointer select-none" @click="changeTagMode()">
                                        <input
                                            class="appearance-none transition-colors cursor-pointer w-14 h-7 rounded-full"
                                            :class="{' bg-gray-900': queriesToSend.tagMode == 'OU', 'bg-gray-600 ': queriesToSend.tagMode == 'ET'}" />
                                        ...
                                        <span
                                            ...
                                            :class="{'translate-x-7 ': queriesToSend.tagMode == 'ET'}" />
                                    </div>
                                </div>
                                <button
                                    ...
                                    {{ button.name }}
                                    <span v-if="isActive(button.slug) && queriesToSend.tagMode == 'ET'" class="text-[10px]">&nbsp;{{ queriesToSend.tagMode }}</span>
                                </button>
                            </div>
                        </div>
                    </div>
        ...
        <div class="text-center text-gray-500 p-8">
            2023 - Notilac - <Link class=" underline dark:hover:text-white hover:text-mydark" :href="'page/mentions'">Mentions</Link>
        </div>
